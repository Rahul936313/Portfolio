...
export function Hero() {
  ...
  return (
    <section
      id="home"
      className="relative min-h-screen flex items-center justify-center overflow-hidden pt-24 pb-12 sm:pt-28 sm:pb-16"
    >
      <div
        className="absolute inset-0 -z-10 opacity-40 transition-all duration-300"
        style={{
          background: `radial-gradient(600px circle at ${mouse.x}% ${mouse.y}%, oklch(0.65 0.27 295 / 0.35), transparent 60%)`,
        }}
      />
      ...
      <div
        className="absolute bottom-12 right-2 sm:right-10 w-40 h-40 sm:w-96 sm:h-96 rounded-full bg-[var(--neon-purple)] opacity-20 blur-3xl animate-blob -z-10"
        style={{ animationDelay: "4s" }}
      />
      <div
        className="hidden sm:block absolute top-1/2 left-1/3 w-80 h-80 rounded-full bg-[var(--neon-pink)] opacity-10 blur-3xl animate-blob -z-10"
      ...
      <div className="max-w-6xl w-full mx-auto px-3 sm:px-6 grid grid-cols-1 md:grid-cols-[1.2fr_1fr] gap-6 sm:gap-8 md:gap-10 items-center">
        ...
        >
          ...
          <h1 className="text-4xl sm:text-6xl md:text-7xl font-bold tracking-tight mb-4">
            <span className="block text-muted-foreground text-xl sm:text-2xl md:text-3xl font-mono mb-2">
              $ whoami
            </span>
          ...
          <p className="text-sm sm:text-base md:text-lg text-muted-foreground max-w-xl mb-8 leading-relaxed">
            Building intelligent, interactive, and impactful digital experiences with AI, machine
            learning, and modern web technology.
          </p>
          <div className="flex flex-wrap gap-2 sm:gap-3">
            <a
              href="#projects"
              className="group inline-flex items-center gap-2 bg-gradient-primary px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl text-sm sm:text-base font-medium glow hover:scale-105 transition-transform"
            >
              View Projects{" "}
            ...
            <a
              href={resumeUrl}
              download
              className="inline-flex items-center gap-2 glass px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl text-sm sm:text-base font-medium hover:border-[var(--neon-cyan)] transition"
            >
              <Download size={16} /> Resume
            </a>
            <a
              href="#contact"
              className="inline-flex items-center gap-2 glass px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl text-sm sm:text-base font-medium hover:border-[var(--neon-purple)] transition"
            >
          ...
          <div className="flex gap-4 mt-8">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="p-2 glass rounded-lg hover:text-[var(--neon-cyan)] transition"
            >
              <Github size={18} />
            </a>
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="p-2 glass rounded-lg hover:text-[var(--neon-blue)] transition"
            >
              <Linkedin size={18} />
            </a>
          </div>
