          ...
          >
            <TiltCard>
              <div className="neon-border rounded-2xl p-5 sm:p-6 h-full flex flex-col group hover:glow-purple transition-shadow">
                <div className={`h-32 sm:h-40 rounded-xl bg-gradient-to-br ${p.gradient} flex items-center justify-center text-5xl sm:text-6xl mb-4 relative overflow-hidden`}>
                ...
                <h3 className="text-lg sm:text-xl font-bold mb-2 group-hover:text-gradient transition">{p.title}</h3>
                <p className="text-xs sm:text-sm text-muted-foreground mb-4 flex-1">{p.desc}</p>
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {p.tech.map((t) => (
                    <span key={t} className="text-[9px] sm:text-[10px] font-mono px-2 py-1 rounded-md glass">{t}</span>
                  ))}
                </div>
